...
macro_rules! build_commands {
    ($($command_type:ty),*) => {
        {
            #[allow(unused_mut)]
            let mut map: $crate::framework::discord::command::CommandMap = std::collections::HashMap::new();
            $(
                let cmd: Box<dyn $crate::framework::discord::command::Command + Send + Sync> =
...
macro_rules! handle_subcommands {
    ($self:expr, $data:expr, $interaction:expr, $ctx:expr) => {{
        let sub_name = $data
            .get_subcommand_name()
            .ok_or(Error::Generic("Could not get subcommand_name".into()))?;
        let sub_data = $data
            .get_subcommand_data()
            .ok_or(Error::Generic("Could not get subcommand_data".into()))?;
