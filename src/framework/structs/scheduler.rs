...
impl Scheduler {
    ...
    pub async fn schedule(&self, event: ScheduledEvent) {
        ...
        worker::console_log!(
            "[ScheduledJob]: Batch started for cron expression '{}'",
            cron_id
        );
        ...
        for (name, trigger) in TRIGGERS.iter() {
            match can_run_trigger(trigger, &event, &self.env, &self.ctx).await {
                Ok(false) => {
                    worker::console_debug!(
                        "[ScheduledJob]: Skipping trigger '{}' (condition not met)",
                        name
                    );
                    continue;
                }
                Err(e) => {
                    worker::console_error!(
                        "[ScheduledJob]: Error checking condition for trigger '{}': {:?}",
                        name,
                        e
                    );
                    continue;
            ...
            if let Err(e) = trigger.execute(&event, &self.env, &self.ctx).await {
                worker::console_error!(
                    "[ScheduledJob]: Trigger '{}' FAILED with error: {:?}",
                    name,
                    e
                );
                continue;
        ...
        worker::console_log!(
            "[ScheduledJob]: Batch completed successfully for '{}'",
            cron_id
        );
    }
}
