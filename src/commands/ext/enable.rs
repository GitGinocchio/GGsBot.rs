...
impl Command for Enable {
    fn name(&self) -> String { "enable".into() }

    fn description(&self) -> String { "Abilita un estensione del bot sul server!".into() }

    fn options(&self) -> Vec<CommandOption> {
        let mut ext = OptionBuilder::new(CommandOptionType::String, "extension", "L'estensione da abilitare")
            .required(true)
            .build();
