...
impl Command for Disable {
    fn name(&self) -> String { "disable".into() }

    fn description(&self) -> String { "Disabilita un estensione del bot sul server!".into() }

    fn options(&self) -> Vec<CommandOption> {
        let mut ext = OptionBuilder::new(CommandOptionType::String, "extension", "L'estensione da disabilitare")
            .required(true)
            .build();
