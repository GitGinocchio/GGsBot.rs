...
impl Command for Teardown {
    fn name(&self) -> String { "teardown".into() }

    fn description(&self) -> String { "Rimuovi un estensione del bot dal server!".into() }

    fn options(&self) -> Vec<CommandOption> {
        let mut ext = OptionBuilder::new(CommandOptionType::String, "extension", "L'estensione da rimuovere")
            .required(true)
            .build();
