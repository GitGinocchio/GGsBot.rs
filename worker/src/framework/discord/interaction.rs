...
impl InteractionExt for Interaction {
            ...
            match command.autocomplete(data, ctx).await {
                ...
                Ok(None) => {
                    return Err(Error::InteractionFailed(
                        "No autocomplete response supplied".into(),
                    ));
                }
                Err(e) => return Err(e),
            }
    ...
    async fn handle_component(
        ...
        {
            return component.handle(self, ctx, data, target.to_string()).await;
        }
    ...
    async fn perform(&self, ctx: &mut RouteContext<()>) -> Result<InteractionResponse, Error> {
        match self.kind {
            ...
            _ => Err(Error::InvalidPayload(
                "Interaction type not supported".into(),
            )),
        }
    }
    ...
    async fn defer(&self, ephemeral: bool) -> Result<(), Error> {
        ...
        if response.status().is_success() {
            ...
            Err(Error::UpstreamError(
                "Discord: Failed to defer interaction".into(),
            ))
        }
    }
    ...
    async fn delete(&self, ctx: &mut RouteContext<()>) -> Result<InteractionResponse, Error> {
        ...
        let Some(ref channel) = self.channel else {
            return Err(Error::InteractionFailed(
                "Can't delete message if channel is None".to_string(),
            ));
        };
        ...
        if response.status().is_success() {
            ...
            Err(Error::UpstreamError(
                format!("Discord API error: {}", response.status()).into(),
            ))
        }
    }
    ...
    async fn edit(&self, response: &InteractionResponse) -> Result<InteractionResponse, Error> {
        ...
        if status.is_success() {
            ...
            Err(Error::UpstreamError(
                format!("Discord API error: {} - {}", status, err_text).into(),
            ))
        }
    }
    ...
    fn is_dev(&self, ctx: &mut RouteContext<()>) -> bool {
        ...
        let dev_id = ctx.var("DISCORD_DEVELOPER_ID").map(|v| v.to_string()).ok();
