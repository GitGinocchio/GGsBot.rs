...
impl NamespacedKv {
    ...
    ) -> Result<ListResponse, KvError> {
        let mut builder =
            self.store
                .list()
                .prefix(format!("{}:{}", self.prefix, prefix.unwrap_or_default()));
    ...
    pub async fn get(&self, key: &str) -> Result<Option<String>, KvError> {
        self.store.get(&self.format_key(key)).text().await
    }
    ...
    pub async fn get_bulk(
        &self,
        keys: &[impl AsRef<str>],
    ) -> Result<HashMap<String, Option<String>>, KvError> {
    ...
    pub async fn duplicate(
        ...
        if let Some(value) = self.get(source_key).await? {
            self.put(dest_key, value, ttl).await?;
            Ok(true)
        } else {
            Ok(false)
        }
